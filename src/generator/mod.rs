...
pub fn generate(path: PathBuf, options: GeneratorOptions) -> Result<()> {
    match &options.layer {
        GenerateLayer::Flat => {
            ...
            generate_entry_from_path_flat_and_concurrently(Arc::new(path.clone()), Arc::new(path.clone()), Arc::new(ignored_files), Arc::new(options), None)?;
        }
        GenerateLayer::Recursive => {
            let ignored_files = get_ignored_files_from_root(path.clone(), options.ignored_files.clone())?;
            generate_entry_from_path_recursively_and_concurrently(Arc::new(path.clone()), Arc::new(path.clone()), Arc::new(ignored_files), Arc::new(options), None)?;
        }
    }
    Ok(())
}
...
fn generate_entry_from_path_recursively_and_concurrently(path: Arc<PathBuf>, root_dir: Arc<PathBuf>, ignored_files: Arc<Gitignore>, options: Arc<GeneratorOptions>, sup_entry_things: Option<(Arc<Mutex<WisphaDirectEntry>>, mpsc::Sender<bool>, mpsc::Sender<bool>)>) -> Result<()> {
    if path.is_dir() {
        ...
        for entry in entries {
            ...
            thread::spawn(move || -> Result<()> {
                ...
                if should_include_entry(&entry, Arc::clone(&cloned_ignored_files), Arc::clone(&cloned_options)) {
                    generate_entry_from_path_recursively_and_concurrently(cloned_path, cloned_root_dir, cloned_ignored_files, cloned_options, Some((cloned_wispha, cloned_tx, cloned_tx_global)))?;
                }
                Ok(())
            });
        }
...
// `path` and `root_dir` is absolute
fn generate_entry_from_path_flat_and_concurrently(path: Arc<PathBuf>, root_dir: Arc<PathBuf>, ignored_files: Arc<Gitignore>, options: Arc<GeneratorOptions>, sup_entry_things: Option<(Arc<Mutex<WisphaIntermediateEntry>>, mpsc::Sender<bool>, mpsc::Sender<bool>)>) -> Result<()> {
    ...
    if path.is_dir() {
        ...
        for entry in fs::read_dir(&*path).or(Err(GeneratorError::DirCannotRead((*path).clone())))? {
            ...
            thread::spawn(move || -> Result<()> {
                let entry = entry.or(Err(GeneratorError::Unexpected))?;
                if should_include_entry(&entry, Arc::clone(&cloned_ignored_files), Arc::clone(&cloned_options)) {
                    generate_entry_from_path_flat_and_concurrently(cloned_path, cloned_root_dir, cloned_ignored_files, cloned_options, Some((cloned_wispha, cloned_tx, cloned_tx_global)))?;
                }
                Ok(())
            });
        }
